...
public class SA00048Validator extends Validator {
	...
	private void validateInput(InvokeElement invoke, OperationElement operation) {
		try {
			VariableHelper variable = new VariableHelper(fileHandler, navigator.getVariableByName(invoke, invoke.getInputVariableAttribute()));
	...
	private void validateOutput(InvokeElement invoke, OperationElement operation) {
		try {
			VariableHelper variable = new VariableHelper(fileHandler, navigator.getVariableByName(invoke, invoke.getOutputVariableAttribute()));
