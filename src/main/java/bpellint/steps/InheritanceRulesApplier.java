...
public class InheritanceRulesApplier {
	...
	public String applyInheritanceRulesTo(InputStream bpelFile)
	...
	private void applyExpressionLanguage(Document doc) {
		...
		String[] elements = {"branches", "condition", "finalCounterValue",
				"for", "from", "joinCondition", "repeatEvery",
				"startCounterValue", "to", "transitionCondition"};

		applyAttributeToElements(doc, attributeName, elements);
	}

	private void applyAttributeToElements(Document doc, String attributeName,
	                                      String[] elements) {
		Attribute expressionLanguage = doc.getRootElement().getAttribute(
				attributeName);
	...
	private void applyExitOnStandardFailure(Document doc) {
		...
		applyAttributeRecursively(root, attribute, attributeName,
				Arrays.asList(elementNames));
	}

	private void applySuppressJoinFailure(Document doc) {
		String attributeName = "suppressJoinFailure";
		String[] elementNames = {"assign", "compensate", "compensateScope",
				"empty", "exit", "flow", "forEach", "if", "invoke", "pick",
				"receive", "repeatUntil", "reply", "rethrow", "scope",
				"sequence", "throw", "validate", "wait", "while"};
	...
	private void applyAttributeRecursively(Element root,
	                                       Attribute exitOnStandardFaultAttribute, String attributeName,
	                                       List<String> elementNames) {
		Elements children = root.getChildElements();
		for (int i = 0; i < children.size(); i++) {
			Element child = children.get(i);
			if (child.getNamespaceURI().equals(
					"http://docs.oasis-open.org/wsbpel/2.0/process/executable")
					&& elementNames.contains(child.getLocalName())) {
				if (child.getAttribute(attributeName) == null) {
					child.addAttribute((Attribute) exitOnStandardFaultAttribute
							.copy());
					// recursion with previous attribute
					applyAttributeRecursively(child,
							exitOnStandardFaultAttribute, attributeName,
							elementNames);
				} else {
					// recursion with new attribute
					applyAttributeRecursively(child,
							child.getAttribute(attributeName), attributeName,
							elementNames);
				}

			} else {
				// recursion with previous attribute
				applyAttributeRecursively(child, exitOnStandardFaultAttribute,
						attributeName, elementNames);
			}
		}
	}
